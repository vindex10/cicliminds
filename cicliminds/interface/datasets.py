...
def get_datasets_for_block(datasets_reg, query):
    ...
    mask = mask & get_scenarios_mask(datasets_reg, mask, True, query)
...
def get_scenarios_mask(datasets_reg, mask, agg, query):
    blocks_with_mask = [(query, mask)]
    scenarios_mask = pd.Series(np.full(datasets_reg.shape[0], False), index=datasets_reg.index)
    for _, partial_mask in expand_model_scenarios(blocks_with_mask, query, agg, datasets_reg):
